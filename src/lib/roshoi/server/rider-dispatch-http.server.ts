  ...
  try {
    ...
    if (request.method === "GET") {
      const rows = await sql<{
        id: string; order_id: string; score: number; distance_m: number | null; eta_seconds: number | null; offered_at: string; expires_at: string;
        restaurant_name: string; restaurant_address: string; zone_name: string; total_paise: number;
      }>`
        select da.id, da.order_id, da.score, da.distance_m, da.eta_seconds, da.offered_at::text,
               (da.offered_at + interval '30 seconds')::text as expires_at,
        ...
        where da.org_id = ${ws.ctx.orgId} and da.rider_id = ${riderId} and da.status = 'OFFERED'
          and o.data_mode = 'PRODUCTION' and o.status = 'READY'
          and da.offered_at > now() - interval '30 seconds'
        order by da.offered_at desc limit 5
      `;
    ...
    const accepted = await sql<{ order_id: string }>`
      with accepted as (
        ...
        where id=${body.offerId} and org_id=${ws.ctx.orgId} and rider_id=${riderId} and status='OFFERED'
    ...
    `;
    if (!accepted[0]) return json({ error: "Offer is no longer available or order was already claimed", code: "OFFER_GONE" }, 409);
