...
export async function handleRiderOrderTransition(request: Request, input: { orderId: string; riderId: string; from: CanonicalOrderStatus; to: CanonicalOrderStatus; reason?: string; idempotencyKey: string; correlationId?: string }): Promise<Response> {
  try {
    ...
    const riderRows = await sql.query<{ id: string }>(`select id from riders where org_id=$1 and user_id=$2 and data_mode='PRODUCTION' and status in ('ACTIVE','ONLINE','BUSY') limit 1`, [ws.ctx.orgId, riderUserId]);
    const rider = riderRows[0];
    if (!rider) throw new ForbiddenError("Rider identity is not bound to an active LIVE rider");
    if (input.riderId !== rider.id) throw new ForbiddenError("Rider identity does not match the authenticated rider");
    ...
    const order = rows[0]; if (!order) throw new ForbiddenError("Order not found"); if (order.data_mode !== "LIVE") throw new Error("Rider LIVE transition endpoint accepts LIVE orders only"); if (input.to !== "RIDER_ASSIGNED" && order.rider_id && order.rider_id !== riderId) throw new ForbiddenError("Rider is not assigned to this order");
