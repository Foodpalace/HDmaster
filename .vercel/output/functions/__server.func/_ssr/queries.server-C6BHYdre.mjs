...
import { n as assertTransition, r as isDelayed } from "./state-machine-C0WNCLu9.mjs";
import { t as cityIdFromSlug } from "./search-vodECYHq.mjs";
import { o as requireHighRisk, s as requirePermission, t as ForbiddenError } from "./rbac-DLs78-ty.mjs";
import { appendAudit, nid } from "./workspace.server-DJpercHs.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/queries.server-C6BHYdre.js
/** Fields that move money. Changing them requires `modify_financial_settings`. */
var FINANCIAL_SETTING_KEYS = [
	"commissionBps",
	"paymentFeeBps",
	"serviceFeePaise",
	"riderBasePaise",
...
function financialSettingsChanged(prev, next) {
...
async function loadRuntimeFlags(orgId) {
	return (await (await getSql()).query(`select key, state, rollout_pct from feature_flags where org_id=$1`, [orgId])).map((r) => ({
		key: r.key,
		state: r.state,
		rolloutPct: n(r.rollout_pct)
	}));
}
async function requireFlag(ws, key) {
	if (!isFlagEnabled(await loadRuntimeFlags(ws.ctx.orgId), key)) throw new ForbiddenError(`Feature flag "${key}" is OFF`);
}
async function rememberIdempotency(ws, key, action, run) {
	if (!key) return run();
	const sql = await getSql();
	const existing = await sql.query(`select response_json from idempotency_keys where key=$1 and org_id=$2`, [key, ws.ctx.orgId]);
	if (existing[0]) try {
	...
	await sql.query(`insert into idempotency_keys (key, org_id, employee_id, action, response_json)
...
async function dashboardPayload(ws) {
	...
	const [kpis, live, alerts, delayed, unassigned, support] = await Promise.all([
       ...
       from orders where org_id = $1 ${city ? "and city_id = $3" : ""}`, params2),
		sql.query(`select
        ...
        (select count(*)::int from orders where org_id=$1 and status in ('RIDER_ASSIGNED','PICKED_UP','ON_THE_WAY','ARRIVING') ${city ? "and city_id=$2" : ""}) as active_del`, params1),
		sql.query(`select id, severity, kind, title, body, created_at from alerts
       ...
       limit 12`, params1),
		sql.query(`select count(*)::int as n from orders
       where org_id=$1 and status in ('PENDING','CONFIRMED','PREPARING','READY','RIDER_ASSIGNED','PICKED_UP','ON_THE_WAY','ARRIVING')
         and promised_at < now() ${citySql.replace("$3", "$2")}`, params1),
		sql.query(`select count(*)::int as n from orders
       where org_id=$1 and status in ('READY','CONFIRMED','PREPARING') and rider_id is null ${citySql.replace("$3", "$2")}`, params1),
		sql.query(`select count(*)::int as n from tickets where org_id=$1 and status in ('OPEN','ASSIGNED','IN_PROGRESS','WAITING') ${city ? "and (city_id is null or city_id=$2)" : ""}`, params1)
	]);
	const k = (Array.isArray(kpis) ? kpis[0] : void 0) ?? {
		orders: 0,
		gmv: 0,
		revenue: 0,
		settlements: 0,
	...
	return {
		...
		today: {
			...
			platformRevenue: canFinance ? {
				value: n(k.revenue),
				label: "SIMULATED"
			} : null,
			restaurantSettlements: canFinance ? {
				value: n(k.settlements),
				label: "SIMULATED"
			} : null,
			riderPayoutExposure: canFinance ? {
				value: n(k.rider_pay),
				label: "SIMULATED"
			} : null,
			refunds: {
			...
			activeRestaurants: {
				value: n(l.active_rst),
				label: "SIMULATED"
			},
			onlineRiders: {
				value: n(l.online_riders),
				label: "SIMULATED"
			},
			activeDeliveries: {
				value: n(l.active_del),
				label: "SIMULATED"
			},
			supportLoad: {
				value: n(support[0]?.n),
...
async function listOrders(ctx, filter) {
	...
	const scopedCity = ctx.cityId ?? (filter.cityId ? cityIdFromSlug(filter.cityId) ?? filter.cityId : void 0);
	...
	const rows = await sql.query(`select o.id, o.status, o.payment_status, o.payment_method, r.name as restaurant, o.restaurant_id,
...
async function getOrder(ctx, id) {
	requirePermission(ctx, "view_orders");
	const sql = await getSql();
	const o = (await sql.query(`select o.*, r.name as restaurant_name, c.display_ref as customer_ref, rd.name as rider_name, z.name as zone_name
     from orders o
     join restaurants r on r.id = o.restaurant_id
     join customers c on c.id = o.customer_id
     left join riders rd on rd.id = o.rider_id
     join zones z on z.id = o.zone_id
     where o.id = $1 and o.org_id = $2 ${ctx.cityId ? "and o.city_id = $3" : ""}`, ctx.cityId ? [
		id,
		ctx.orgId,
		ctx.cityId
	] : [id, ctx.orgId]))[0];
	if (!o) throw new ForbiddenError("Order not found");
	const items = await sql.query(`select name, qty, unit_paise from order_items where order_id=$1`, [id]);
	const events = await sql.query(`select action, from_status, to_status, note, created_at from order_events where order_id=$1 order by created_at`, [id]);
	const financeOk = ctx.permissions.includes("view_finance") || ctx.actingRoleKey === "SUPER_ADMIN";
	const ledger = financeOk ? await sql.query(`select kind, source, rule_key, amount_paise, note, created_at from ledger_entries where order_id=$1`, [id]) : [];
	return {
		id: str(o.id),
		status: str(o.status),
...
async function interveneOrder(ws, input) {
	return rememberIdempotency(ws, input.idempotencyKey, `order.${input.action}`, async () => {
		...
		if (input.action === "cancel") {
			requireHighRisk(ws.ctx, "cancel_orders", input.reason, {
				orgId: ws.ctx.orgId,
				cityId: String(order.cityId)
			});
			assertTransition(order.status, "CANCELLED");
			await sql.query(`update orders set status='CANCELLED', cancelled_at=now(), cancel_reason=$3 where id=$1 and org_id=$2`, [
				input.orderId,
				ws.ctx.orgId,
				input.reason
			]);
		} else if (input.action === "refund") {
			requireHighRisk(ws.ctx, "issue_refunds", input.reason, {
				orgId: ws.ctx.orgId,
				cityId: String(order.cityId)
			});
			const amount = input.amountPaise ?? order.totalPaise;
			if (amount > ws.settings.refundLimitPaise) throw new ForbiddenError("Refund exceeds configured limit");
			if (amount <= 0 || amount > order.totalPaise) throw new ForbiddenError("Refund amount is out of range");
			const next = "REFUND_PENDING";
			assertTransition(order.status, next);
			await sql.query(`update orders set status=$3, refund_paise=$4 where id=$1 and org_id=$2`, [
				input.orderId,
				ws.ctx.orgId,
				next,
				amount
			]);
			await sql.query(`insert into ledger_entries (id, org_id, order_id, restaurant_id, party, kind, source, rule_key, amount_paise, note)
         values ($1,$2,$3,$4,'CUSTOMER','refund','support.refund','authorized_refund',$5,$6)`, [
			...
			await sql.query(`insert into dispatch_requests (id, org_id, order_id, requested_rider_id, status, reason, created_by_employee_id)
         values ($1,$2,$3,$4,'REQUESTED',$5,$6)`, [
			...
			const to = "RIDER_ASSIGNED";
			if (order.status !== "RIDER_ASSIGNED") assertTransition(order.status, to);
			await sql.query(`update orders set rider_id=$3, status=$4 where id=$1 and org_id=$2`, [
				input.orderId,
				ws.ctx.orgId,
				input.riderId,
				to
			...
			await sql.query(`insert into tickets (id, org_id, city_id, queue, category, status, priority, subject, order_id, sla_minutes, data_mode)
         values ($1,$2,$3,'customer','order_issue','OPEN','HIGH',$4,$5,$6,'SIMULATED')`, [
				nid("tkt"),
				ws.ctx.orgId,
				order.cityId,
				`Escalation ${input.orderId}`,
				input.orderId,
				ws.settings.supportSlaMinutes
			]);
		}
		await sql.query(`insert into order_events (id, org_id, order_id, actor_employee_id, from_status, to_status, action, note)
       values ($1,$2,$3,$4,$5,$6,$7,$8)`, [
			...
			`admin.${input.action}`,
			input.action === "assign_rider" ? `Dispatch REQUEST to core (simulated locally): ${input.reason}` : input.reason
		]);
		await appendAudit({
			orgId: ws.ctx.orgId,
...
async function listRestaurants(ctx, q, status) {
	...
	return (await sql.query(`select r.*, (select count(*)::int from orders o where o.restaurant_id=r.id) as order_count
     from restaurants r where ${where} order by name`, params)).map((r) => ({
		id: String(r.id),
		name: String(r.name),
...
async function getRestaurant(ctx, id) {
	requirePermission(ctx, "view_restaurants");
	const sql = await getSql();
	const r = (await sql.query(`select * from restaurants where id=$1 and org_id=$2 ${ctx.cityId ? "and city_id=$3" : ""}`, ctx.cityId ? [
		id,
		ctx.orgId,
		ctx.cityId
	] : [id, ctx.orgId]))[0];
	if (!r) throw new ForbiddenError("Restaurant not found");
	const menu = await sql.query(`select id, category, name, price_paise, available, veg from menu_items where restaurant_id=$1`, [id]);
	const s = (await sql.query(`select count(*)::int as orders,
	...
	return {
		...
		contributionEstimate: finance ? {
			value: Math.round(n(s?.gmv) * (n(r.commission_bps) / 1e4) * .55),
			label: "ESTIMATE"
		} : null,
		dataMode: str(r.data_mode)
	};
}
async function setRestaurantStatus(ws, id, status, reason) {
	const current = await getRestaurant(ws.ctx, id);
	const perm = status === "SUSPENDED" || status === "CLOSED" ? "suspend_restaurants" : status === "ACTIVE" || status === "APPROVED" ? "approve_restaurants" : "approve_restaurants";
	requireHighRisk(ws.ctx, perm, reason, {
		orgId: ws.ctx.orgId,
		cityId: String(current.cityId)
	});
	await (await getSql()).query(`update restaurants set status=$3 where id=$1 and org_id=$2`, [
		id,
		ws.ctx.orgId,
		status
	]);
...
async function listRiders(ctx, q) {
	...
	const rows = await sql.query(`select * from riders where ${where} order by online desc, name`, params);
	const finance = ctx.permissions.includes("view_finance") || ctx.actingRoleKey === "SUPER_ADMIN";
	return rows.map((r) => ({
		id: String(r.id),
		name: String(r.name),
...
async function getRider(ctx, id) {
	const rider = (await listRiders(ctx)).find((r) => r.id === id);
	if (!rider) throw new ForbiddenError("Rider not found");
	const hist = await (await getSql()).query(`select id, status, total_paise, placed_at from orders where rider_id=$1 order by placed_at desc limit 20`, [id]);
	return {
		...rider,
...
async function setRiderStatus(ws, id, status, reason) {
	requireHighRisk(ws.ctx, status === "SUSPENDED" ? "suspend_riders" : "approve_riders", reason);
	const sql = await getSql();
	const prev = await sql.query(`select status, city_id from riders where id=$1 and org_id=$2`, [id, ws.ctx.orgId]);
...
async function listCustomers(ctx, q) {
	...
	return (await sql.query(`select id, display_ref, phone_masked, status, loyalty_tier, order_count, risk_score, city_id, last_order_at, data_mode
     from customers where ${where} order by display_ref limit 80`, params)).map((c) => ({
		id: String(c.id),
...
async function getCustomer(ctx, id) {
	requirePermission(ctx, "view_customers");
	const sql = await getSql();
	const c = (await sql.query(`select id, display_ref, phone_masked, status, loyalty_tier, order_count, risk_score, city_id, last_order_at, data_mode
     from customers where id=$1 and org_id=$2 ${ctx.cityId ? "and city_id=$3" : ""}`, ctx.cityId ? [
		id,
		ctx.orgId,
		ctx.cityId
	] : [id, ctx.orgId]))[0];
	if (!c) throw new ForbiddenError("Customer not found");
	const orders = await sql.query(`select o.id, o.status, o.total_paise, o.placed_at, r.name as restaurant
     from orders o join restaurants r on r.id=o.restaurant_id
     where o.customer_id=$1 and o.org_id=$2 ${ctx.cityId ? "and o.city_id=$3" : ""}
     order by o.placed_at desc limit 20`, ctx.cityId ? [
		id,
		ctx.orgId,
		ctx.cityId
	] : [id, ctx.orgId]);
...
async function updateCustomer(ws, input) {
	requireHighRisk(ws.ctx, "edit_customers", input.reason);
	const prev = await getCustomer(ws.ctx, input.id);
	await (await getSql()).query(`update customers set status=coalesce($3, status), loyalty_tier=coalesce($4, loyalty_tier) where id=$1 and org_id=$2`, [
		input.id,
		ws.ctx.orgId,
		input.status ?? null,
		input.loyaltyTier ?? null
	]);
	await appendAudit({
		orgId: ws.ctx.orgId,
		employeeId: ws.ctx.employeeId,
...
async function listTickets(ctx, queue, status) {
	...
	return (await sql.query(`select * from tickets where ${where} order by opened_at desc limit 80`, params)).map((t) => ({
		id: String(t.id),
...
async function getTicket(ctx, id) {
	requirePermission(ctx, "manage_support");
	const sql = await getSql();
	const t = (await sql.query(`select * from tickets where id=$1 and org_id=$2`, [id, ctx.orgId]))[0];
	...
	const msgs = await sql.query(`select id, visibility, author_type, body, created_at from ticket_messages where ticket_id=$1 order by created_at`, [id]);
	return {
		id: str(t.id),
		queue: str(t.queue),
		...
		slaBreached: Boolean(!t.resolved_at && t.opened_at && Date.now() - new Date(String(t.opened_at)).getTime() > n(t.sla_minutes) * 6e4),
		messages: msgs.map((m) => ({
			id: m.id,
...
async function mutateTicket(ws, input) {
	return rememberIdempotency(ws, input.idempotencyKey, `ticket.${input.action}`, async () => {
		requirePermission(ws.ctx, "manage_support");
		const sql = await getSql();
		const t = await getTicket(ws.ctx, input.id);
		if (input.action === "assign") await sql.query(`update tickets set assigned_employee_id=$2, status='ASSIGNED' where id=$1`, [input.id, input.employeeId ?? ws.ctx.employeeId]);
		else if (input.action === "note" || input.action === "reply") {
			await sql.query(`insert into ticket_messages (id, ticket_id, org_id, visibility, author_type, author_employee_id, body)
         values ($1,$2,$3,$4,'employee',$5,$6)`, [
			...
			if (input.action === "reply") await sql.query(`update tickets set status='WAITING' where id=$1`, [input.id]);
		} else if (input.action === "resolve") await sql.query(`update tickets set status='RESOLVED', resolved_at=now(), resolution_code=$2 where id=$1`, [input.id, input.resolutionCode ?? input.body ?? "resolved"]);
		else if (input.action === "reopen") await sql.query(`update tickets set status='OPEN', resolved_at=null where id=$1`, [input.id]);
		await appendAudit({
			orgId: ws.ctx.orgId,
			employeeId: ws.ctx.employeeId,
			userId: ws.ctx.userId,
...
async function financeSummary(ctx) {
	...
	const r = (await sql.query(`select
      ...
      coalesce(sum(total_paise),0)::int as gmv,
      count(*)::int as orders
     from orders where org_id=$1 ${city ? "and city_id=$2" : ""}`, city ? [ctx.orgId, city] : [ctx.orgId]))[0] ?? {
		commission: 0,
		delivery: 0,
		service: 0,
		rider: 0,
...
async function settlementRows(ctx, party) {
	requirePermission(ctx, "view_finance");
	const sql = await getSql();
	if (party === "RESTAURANT") return (await sql.query(`select o.restaurant_id, r.name,
              count(*)::int as orders,
       ...
       order by settlement desc`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId])).map((x) => ({
		id: x.restaurant_id,
		name: x.name,
		orders: n(x.orders),
		foodPaise: n(x.food),
	...
	return (await sql.query(`select o.rider_id, rd.name,
            count(*)::int as orders,
     ...
     order by payout desc`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId])).map((x) => ({
		id: x.rider_id,
		name: x.name,
		orders: n(x.orders),
...
async function listEmployees(ctx) {
	requirePermission(ctx, "manage_users");
	return (await (await getSql()).query(`select id, email, name, role_key, department, city_id, area_id, status, mfa_ready, last_active_at, assumed_role_key, access_expires_at
     from employees where org_id=$1 order by created_at`, [ctx.orgId])).map((e) => ({
		id: String(e.id),
...
async function inviteEmployee(ws, input) {
	requirePermission(ws.ctx, "manage_users");
	if (input.roleKey === "SUPER_ADMIN" && ws.ctx.actingRoleKey !== "SUPER_ADMIN") throw new ForbiddenError("Cannot grant Super Admin");
	const sql = await getSql();
	const id = nid("emp");
	await sql.query(`insert into employees (id, org_id, email, name, role_key, department, city_id, status, custom_permissions_json, invited_by)
...
async function updateEmployee(ws, input) {
	if (input.id !== ws.ctx.employeeId) requireHighRisk(ws.ctx, "manage_roles", input.reason);
	else if (input.assumedRoleKey !== void 0) requirePermission(ws.ctx, "assume_role");
	const sql = await getSql();
	const prev = await sql.query(`select * from employees where id=$1 and org_id=$2`, [input.id, ws.ctx.orgId]);
	...
	const nextCity = input.cityId === void 0 ? prev[0].city_id : input.cityId;
	const nextDept = input.department ?? String(prev[0].department);
	const nextCustom = input.customPermissions !== void 0 ? JSON.stringify(input.customPermissions) : prev[0].custom_permissions_json;
	const nextAssume = input.assumedRoleKey === void 0 ? prev[0].assumed_role_key : input.assumedRoleKey;
	const nextMfa = input.mfaReady === void 0 ? n(prev[0].mfa_ready) : input.mfaReady ? 1 : 0;
	await sql.query(`update employees set role_key=$3, status=$4, city_id=$5, department=$6, custom_permissions_json=$7, assumed_role_key=$8, mfa_ready=$9
...
async function listAudit(ctx, q) {
	requirePermission(ctx, "view_audit_logs");
	return (await (await getSql()).query(`select id, employee_id, role_key, action, target_type, target_id, previous_json, new_json, reason, created_at
     from audit_logs where org_id=$1 ${q ? "and (action ilike $2 or target_id ilike $2)" : ""}
     order by created_at desc limit 100`, q ? [ctx.orgId, `%${q}%`] : [ctx.orgId])).map((a) => ({
		id: String(a.id),
...
async function listFlags(ctx) {
	requirePermission(ctx, "manage_feature_flags");
	const rows = await (await getSql()).query(`select key, state, rollout_pct, city_id, notes from feature_flags where org_id=$1 order by key`, [ctx.orgId]);
...
async function setFlag(ws, key, state, rolloutPct, reason) {
	requireHighRisk(ws.ctx, "manage_feature_flags", reason);
	await (await getSql()).query(`insert into feature_flags (id, org_id, key, state, rollout_pct, notes)
     values ($1,$2,$3,$4,$5,$6)
...
async function getBranding(ctx) {
	const row = (await (await getSql()).query(`select * from branding where org_id=$1`, [ctx.orgId]))[0];
	if (!row) return null;
	const out = {};
	for (const [k, v] of Object.entries(row)) out[k] = v == null ? "" : String(v);
	return out;
}
async function saveBranding(ws, patch, reason) {
	...
	await sql.query(`update branding set ${sets}, updated_at=now() where org_id=$1`, [ws.ctx.orgId, ...values]);
	await appendAudit({
		orgId: ws.ctx.orgId,
		employeeId: ws.ctx.employeeId,
		userId: ws.ctx.userId,
...
async function getSettings(ctx) {
	...
	const rows = await (await getSql()).query(`select settings_json from platform_settings where org_id=$1`, [ctx.orgId]);
...
async function saveSettings(ws, settings, reason) {
	requireHighRisk(ws.ctx, "manage_platform_settings", reason);
	const sql = await getSql();
	const prev = await getSettings(ws.ctx);
	if (financialSettingsChanged(prev, settings)) requireHighRisk(ws.ctx, "modify_financial_settings", reason);
	const flags = await loadRuntimeFlags(ws.ctx.orgId);
	if (settings.codEnabled && !prev.codEnabled && !isFlagEnabled(flags, "cod")) throw new ForbiddenError("Feature flag \"cod\" is OFF — cannot enable COD");
	if (settings.otpRequired && !prev.otpRequired && !isFlagEnabled(flags, "delivery_otp")) throw new ForbiddenError("Feature flag \"delivery_otp\" is OFF — cannot require OTP");
	await sql.query(`update platform_settings set settings_json=$2, updated_at=now(), updated_by_employee_id=$3 where org_id=$1`, [
		ws.ctx.orgId,
...
async function listPromotions(ctx) {
	requirePermission(ctx, "manage_promotions");
	return (await (await getSql()).query(`select * from promotions where org_id=$1 order by created_at desc`, [ctx.orgId])).map((p) => ({
		id: String(p.id),
		name: String(p.name),
		kind: String(p.kind),
...
async function savePromotion(ws, input) {
	requirePermission(ws.ctx, "manage_promotions");
	if (input.status === "ACTIVE") {
		const flags = await loadRuntimeFlags(ws.ctx.orgId);
		if (input.kind === "LOYALTY" && !isFlagEnabled(flags, "loyalty")) throw new ForbiddenError("Feature flag \"loyalty\" is OFF");
	}
	if (input.status === "ACTIVE" && !input.capCount && !input.maxDiscountPaise) throw new ForbiddenError("Active promotions require a cap or max discount — unlimited discounts are blocked");
	const sql = await getSql();
	const estimated = (input.fixedPaise ?? 0) * (input.capCount ?? 100) || Math.round((input.percentBps ?? 0) / 1e4 * 3e4 * (input.capCount ?? 100));
	const id = input.id ?? nid("promo");
	await sql.query(`insert into promotions (
	...
	return {
		id,
		estimatedCostPaise: estimated,
		label: "ESTIMATE"
	};
}
async function listCms(ctx) {
	requirePermission(ctx, "manage_cms");
	return (await (await getSql()).query(`select * from cms_entries where org_id=$1 order by surface, sort_order`, [ctx.orgId])).map((c) => ({
		id: String(c.id),
...
async function saveCms(ws, input) {
	requirePermission(ws.ctx, "manage_cms");
	if (input.sponsored) {
		if (!isFlagEnabled(await loadRuntimeFlags(ws.ctx.orgId), "advertising")) throw new ForbiddenError("Feature flag \"advertising\" is OFF — cannot publish sponsored content");
	}
	const sql = await getSql();
	const id = input.id ?? nid("cms");
	await sql.query(`insert into cms_entries (id, org_id, surface, slot, title, body, sponsored, sort_order, status)
...
async function listZones(ctx) {
	...
	return (await (await getSql()).query(`select z.*, c.name as city_name from zones z join cities c on c.id=z.city_id
     where z.org_id=$1 ${ctx.cityId ? "and z.city_id=$2" : ""} order by z.name`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId])).map((z) => ({
		id: String(z.id),
		name: String(z.name),
...
async function saveZone(ws, input) {
	requirePermission(ws.ctx, "manage_delivery_zones", {
		orgId: ws.ctx.orgId,
		cityId: input.cityId
	});
	const sql = await getSql();
	const id = input.id ?? nid("zone");
	await sql.query(`insert into zones (id, org_id, city_id, name, geometry_json, delivery_fee_paise, min_order_paise, max_radius_km, eta_minutes)
     ...
     on conflict (id) do update set name=excluded.name, delivery_fee_paise=excluded.delivery_fee_paise,
       min_order_paise=excluded.min_order_paise, max_radius_km=excluded.max_radius_km, eta_minutes=excluded.eta_minutes, geometry_json=excluded.geometry_json`, [
		...
		input.cityId,
		input.name,
		input.geometryJson ?? JSON.stringify({
			type: "radius",
			radiusKm: input.maxRadiusKm
		}),
...
async function listCities(ctx) {
	return await (await getSql()).query(`select id, name, state from cities where org_id=$1 order by name`, [ctx.orgId]);
}
async function listKyc(ctx) {
	requirePermission(ctx, "view_kyc");
	return (await (await getSql()).query(`select k.*, coalesce(r.city_id, rd.city_id) as city_id
     from kyc_cases k
     ...
     order by k.updated_at desc`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId])).map((k) => ({
		id: String(k.id),
		subjectType: String(k.subject_type),
		subjectId: String(k.subject_id),
		status: String(k.status),
		notes: k.notes ? String(k.notes) : "",
		documentRefs: JSON.parse(String(k.document_refs_json || "[]")),
		cityId: k.city_id ? String(k.city_id) : null
	}));
}
async function reviewKyc(ws, id, status, notes) {
	requireHighRisk(ws.ctx, "review_kyc", notes);
	const sql = await getSql();
	const prev = await sql.query(`select status from kyc_cases where id=$1 and org_id=$2`, [id, ws.ctx.orgId]);
	await sql.query(`update kyc_cases set status=$3, notes=$4, reviewer_employee_id=$5, updated_at=now() where id=$1 and org_id=$2`, [
		id,
		ws.ctx.orgId,
		status,
		notes,
...
async function listRisk(ctx) {
	requirePermission(ctx, "view_risk");
	return (await (await getSql()).query(`select rs.*, coalesce(c.city_id, rd.city_id, r.city_id) as city_id
     ...
     order by rs.score desc`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId])).map((r) => ({
		id: String(r.id),
...
async function listNotifications(ctx) {
	...
	return (await (await getSql()).query(`select * from notifications where org_id=$1 order by created_at desc limit 50`, [ctx.orgId])).map((n0) => ({
		id: String(n0.id),
...
async function listLoyalty(ctx) {
	requirePermission(ctx, "manage_promotions");
	return (await (await getSql()).query(`select * from loyalty_programs where org_id=$1`, [ctx.orgId])).map((l) => ({
		id: String(l.id),
		name: String(l.name),
		kind: String(l.kind),
...
async function dispatchBoard(ctx) {
	requirePermission(ctx, "view_orders");
	const unassigned = await listOrders(ctx, {
		status: "READY",
		limit: 40
	});
	const preparing = await listOrders(ctx, {
		status: "PREPARING",
		limit: 20
	});
	const riders = await listRiders(ctx);
	const requests = await (await getSql()).query(`select id, order_id, requested_rider_id, status, reason, created_at
     from dispatch_requests where org_id=$1 order by created_at desc limit 25`, [ctx.orgId]).catch(() => []);
	return {
...
async function liveBoard(ctx) {
	requirePermission(ctx, "view_orders");
	const rows = await (await getSql()).query(`select status, count(*)::int as n from orders
     where org_id=$1 ${ctx.cityId ? "and city_id=$2" : ""}
       and placed_at > now() - interval '24 hours'
     group by status`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId]);
	const counts = {};
	...
	const tracking = isFlagEnabled(await loadRuntimeFlags(ctx.orgId), "live_tracking");
	return {
		counts,
		orders,
		riders: riders.map((r) => ({
...
async function tickSimulation(ws) {
	requirePermission(ws.ctx, "modify_orders");
	const flags = await loadRuntimeFlags(ws.ctx.orgId);
	const sql = await getSql();
	const advancing = await sql.query(`select id, status from orders
     where org_id=$1 and status in ('PENDING','CONFIRMED','PREPARING','READY','RIDER_ASSIGNED','PICKED_UP','ON_THE_WAY','ARRIVING')
     order by placed_at asc limit 3`, [ws.ctx.orgId]);
	const nextMap = {
		PENDING: "CONFIRMED",
		CONFIRMED: "PREPARING",
		PREPARING: "READY",
	...
	for (const o of advancing) {
		...
		await sql.query(`insert into order_events (id, org_id, order_id, from_status, to_status, action, note)
       values ($1,$2,$3,$4,$5,'sim.tick',$6)`, [
			...
			o.status,
			to,
			isFlagEnabled(flags, "delivery_otp") && to === "DELIVERED" ? "SIMULATED clock advance (delivery OTP flag ON)" : "SIMULATED clock advance"
		]);
	}
	return {
...
async function profitability(ctx) {
	requirePermission(ctx, "view_finance");
	const sql = await getSql();
	const byRst = await sql.query(`select o.restaurant_id, r.name,
     ...
     group by o.restaurant_id, r.name`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId]);
	const byZone = await sql.query(`select o.zone_id, z.name,
            coalesce(sum(o.total_paise),0)::int as gmv,
     ...
     where o.org_id=$1 ${ctx.cityId ? "and o.city_id=$2" : ""}
     group by o.zone_id, z.name`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId]);
	const restaurants = byRst.map((r) => {
		const contrib = n(r.commission) - n(r.discount) - Math.round(n(r.payout) * .35) - n(r.refunds);
		return {
			id: r.restaurant_id,
			name: r.name,
			orders: n(r.orders),
...
async function analyticsSeries(ctx) {
	requirePermission(ctx, "view_analytics");
	return {
		series: (await (await getSql()).query(`select to_char(placed_at, 'YYYY-MM-DD') as day,
            count(*)::int as orders,
            coalesce(sum(total_paise),0)::int as gmv
     from orders where org_id=$1 ${ctx.cityId ? "and city_id=$2" : ""}
     group by 1 order by 1`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId])).map((r) => ({
			day: r.day,
			orders: n(r.orders),
			gmv: n(r.gmv)
		})),
		label: "SIMULATED",
		grain: "RAW_AGGREGATED"
	};
}
async function ceoBrief(ctx) {
	...
	const backlog = await (await getSql()).query(`select count(*)::int as n from tickets
     where org_id=$1 and status not in ('RESOLVED','CLOSED') ${ctx.cityId ? "and (city_id is null or city_id=$2)" : ""}`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId]);
	...
	return {
		...
		briefing: {
			...
			finance: worst ? `Weakest estimated contribution is ${worst.name} at ${worst.contribution} paise (ESTIMATE from ledger-derived commission minus refunds and a share of rider cost).` : "Not enough restaurant volume to rank contribution.",
			recommended: [
				delayedN > 0 ? `Clear ${delayedN} delayed orders before expanding coverage.` : "No delayed-order alert right now.",
				contrib < 0 ? "Do not drop commission without a volume offset — estimated contribution is already negative (MODEL)." : "Do not drop commission from 10% to 8% without a volume offset (MODEL).",
...
async function saveLoyalty(ws, input) {
	requirePermission(ws.ctx, "manage_promotions");
	await requireFlag(ws, "loyalty");
	if (!input.capPaise || input.capPaise <= 0) throw new ForbiddenError("Loyalty programs require a rupee cap — unlimited earn is blocked");
	const sql = await getSql();
	const id = input.id ?? nid("loy");
	await sql.query(`insert into loyalty_programs (id, org_id, name, kind, earn_bps, cap_paise, status, rules_json)
     values ($1,$2,$3,$4,$5,$6,$7,$8)
     on conflict (id) do update set name=excluded.name, kind=excluded.kind, earn_bps=excluded.earn_bps,
       cap_paise=excluded.cap_paise, status=excluded.status, rules_json=excluded.rules_json`, [
		...
		input.status,
		JSON.stringify({
			abuseCapPerDay: input.abuseCapPerDay,
			minOrderPaise: 15e3,
			expiryDays: 90
		})
	]);
...
async function listCampaigns(ctx) {
	requirePermission(ctx, "manage_cms");
	return (await (await getSql()).query(`select * from marketing_campaigns where org_id=$1 order by created_at desc`, [ctx.orgId])).map((c) => ({
		id: String(c.id),
		name: String(c.name),
...
async function saveCampaign(ws, input) {
	requirePermission(ws.ctx, "manage_cms");
	const sql = await getSql();
	const id = input.id ?? nid("cmp");
	await sql.query(`insert into marketing_campaigns (id, org_id, name, channel, audience, budget_paise, status, notes)
     values ($1,$2,$3,$4,$5,$6,$7,$8)
	...
	return {
		id,
		estimatedCostPaise: input.budgetPaise,
		label: "ESTIMATE"
	};
}
async function reportsPayload(ctx) {
	requirePermission(ctx, "view_analytics");
	const series = await analyticsSeries(ctx);
	const sql = await getSql();
	const byCity = await sql.query(`select o.city_id, c.name, count(*)::int as orders, coalesce(sum(o.total_paise),0)::int as gmv
     from orders o join cities c on c.id=o.city_id
     where o.org_id=$1 ${ctx.cityId ? "and o.city_id=$2" : ""}
     group by o.city_id, c.name`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId]);
	const byStatus = await sql.query(`select status, count(*)::int as n from orders where org_id=$1 ${ctx.cityId ? "and city_id=$2" : ""} group by status`, ctx.cityId ? [ctx.orgId, ctx.cityId] : [ctx.orgId]);
	return {
...
async function listSettlementBatches(ctx, party) {
	requirePermission(ctx, "view_finance");
	const base = await settlementRows(ctx, party);
	const rows = await (await getSql()).query(`select * from settlement_batches where org_id=$1 and party_type=$2 order by created_at desc`, [ctx.orgId, party]);
...
async function approveSettlement(ws, id, decision, reason) {
	requireHighRisk(ws.ctx, "approve_settlements", reason);
	const sql = await getSql();
	const prev = await sql.query(`select status, party_name from settlement_batches where id=$1 and org_id=$2`, [id, ws.ctx.orgId]);
	...
	await sql.query(`update settlement_batches set status=$3, approved_by=$4, approved_at=now(), reason=$5 where id=$1 and org_id=$2`, [
		id,
		ws.ctx.orgId,
		decision,
		ws.ctx.employeeId,
	...
	return {
		ok: true,
		note: "Approval recorded. Payout is not executed from Command."
	};
}
async function queueNotification(ws, input) {
	...
	if (!isFlagEnabled(flags, channelFlag)) throw new ForbiddenError(`Feature flag "${channelFlag}" is OFF`);
	const sql = await getSql();
	const id = nid("nt");
	await sql.query(`insert into notifications (id, org_id, channel, template_key, audience, status, provider, provider_confirmed)
...
//#endregion
export { analyticsSeries, approveSettlement, ceoBrief, dashboardPayload, dispatchBoard, financeSummary, getBranding, getCustomer, getOrder, getRestaurant, getRider, getSettings, getTicket, interveneOrder, inviteEmployee, listAudit, listCampaigns, listCities, listCms, listCustomers, listEmployees, listFlags, listKyc, listLoyalty, listNotifications, listOrders, listPromotions, listRestaurants, listRiders, listRisk, listSettlementBatches, listTickets, listZones, liveBoard, loadRuntimeFlags, mutateTicket, profitability, queueNotification, reportsPayload, reviewKyc, saveBranding, saveCampaign, saveCms, saveLoyalty, savePromotion, saveSettings, saveZone, serializeEmployee, setFlag, setRestaurantStatus, setRiderStatus, settlementRows, systemHealth, tickSimulation, updateCustomer, updateEmployee };
